...
const API_BASE =
  ...
  "https://dmc-server-origin.onrender.com";
...
function App() {
    ...
    const timer = setInterval(async () => {
      try {
        ...
        if (data.status === "completed") {
          setStatusMsg("✅ Video completed and sent.");
          clearInterval(timer);
  ...
  const buildAiPrompt = () => {
    ...
    return [
      ...
      "Write a short emotional spoken message in natural English.",
      "Keep it warm, concise, and suitable for a short video."
    ]
      .filter(Boolean)
      .join(" ");
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "photo") {
        ...
        res = await fetch(`${API_BASE}/create-video`, {
          method: "POST",
          body: formData
        });
      } else {
